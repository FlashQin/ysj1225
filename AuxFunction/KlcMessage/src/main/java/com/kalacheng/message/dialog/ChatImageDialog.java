...
import com.kalacheng.message.adapter.ChatImagePreviewAdapter;
...
public class ChatImageDialog extends PopupWindow {
    ...
    public ChatImageDialog(Context context, View parent) {
        ...
        mScreenHeight = DpUtil.getScreenHeight();
    ...
    private View initView() {
        View v = LayoutInflater.from(mContext).inflate(R.layout.layout_chat_image, null);
    ...
    public void show(ImChatImageBean bean, String url, int x, int y, int imageWidth, int imageHeight, Drawable drawable) {
        if (mCover == null || bean == null || TextUtils.isEmpty(url) || imageWidth <= 0 || imageHeight <= 0 || drawable == null) {
            return;
        }
