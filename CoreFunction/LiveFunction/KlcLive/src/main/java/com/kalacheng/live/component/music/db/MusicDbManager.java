package com.kalacheng.live.component.music.db;
...
public class MusicDbManager {
     ...
     * @param bean
     */
    public void save(ApiMusic bean) {
        SQLiteDatabase db = mDbHelper.getWritableDatabase();
        if (db.isOpen()) {
            Cursor c = db.rawQuery(MusicDbHelper.QUERY + "'" + bean.id + "'", null);
            if (c.moveToNext()) {//如果存在，执行update
                SQLiteStatement st = db.compileStatement(MusicDbHelper.UPDATE);
                db.beginTransaction();
                try {
                    st.bindString(1, bean.name);
                    st.bindString(2, bean.authors);
                    st.bindString(3, bean.id);
                ...
                db.beginTransaction();
                try {
                    st.bindString(1, bean.id);
                    st.bindString(2, bean.name);
                    st.bindString(3, bean.authors);
                    st.executeInsert();
                    db.setTransactionSuccessful();
                } finally {
                    db.endTransaction();
     ...
     * @return
     */
    public List<ApiMusic> queryList() {
        SQLiteDatabase db = mDbHelper.getWritableDatabase();
        if (db.isOpen()) {
            List<ApiMusic> list = new ArrayList<>();
            Cursor c = null;
            try {
                c = db.rawQuery(MusicDbHelper.QUERY_LIST, null);
                while (c.moveToNext()) {
                    ApiMusic bean = new ApiMusic();
                    bean.id = c.getString(c.getColumnIndex(MusicDbHelper.ID));
                    bean.name = c.getString(c.getColumnIndex(MusicDbHelper.NAME));
                    bean.authors = c.getString(c.getColumnIndex(MusicDbHelper.ARTIST));
                    bean.progress = 100;
                    list.add(bean);
                }
            } finally {
