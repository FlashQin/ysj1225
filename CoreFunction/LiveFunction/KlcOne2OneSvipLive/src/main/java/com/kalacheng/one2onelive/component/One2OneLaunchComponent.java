...
public class One2OneLaunchComponent extends BaseMVVMViewHolder<One2oneLiveLaunchBinding, One2OneLaunchViewModel> implements LiveBundle.onLiveSocket, View.OnClickListener {
    //来视频铃声
    ...
    public One2OneLaunchComponent(Context context, ViewGroup parentView) {
    ...
    protected void init() {
        ...
        //接通视频
//        if (LiveBundle.getInstance().isHaveCallBack(LiveConstants.LM_OOOLiveLinkOK)){
//            LiveBundle.getInstance().removeNullMsgListener(LiveConstants.LM_OOOLiveLinkOK);
//        }
        LiveBundle.getInstance().addSimpleMsgListener(LiveConstants.LM_OOOLiveLinkOK, new MsgListener.SimpleMsgListener() {
            ...
            public void onMsg(Object o) {
                ...
                LiveConstants.FEEUID = anchorinfo.feeId;
        ...
        LiveBundle.getInstance().addSimpleMsgListener(LiveConstants.LM_OOOLiveHangUp, new MsgListener.SimpleMsgListener() {
            ...
            public void onMsg(Object o) {
                ...
                if (o == null) return;
                if (((Long) o) == 12) {
                    ImMessageUtil.getInstance().sendCallMessage(viewModel.bean.get().userId, 3, 0, true);

                } else {
                    ImMessageUtil.getInstance().sendCallMessage(viewModel.bean.get().userId, 2, 0, true);
                }
            }
    ...
    public void initView() {
        binding.ConversationUnit.setText(SpUtil.getInstance().getCoinUnit() + "/分钟");
        ...
        ImageLoader.display(viewModel.bean.get().thumb, binding.bgImage, R.mipmap.ic_launcher, R.mipmap.ic_launcher);
        ...
        if (LiveConstants.mIsOOOSend == true) {
            ...
            binding.refuseLin.setVisibility(View.VISIBLE);
            binding.OneVoiceLaunchLoading.setText("你正在与" + viewModel.bean.get().username + "视频...");
        } else {
            binding.accept.setVisibility(View.VISIBLE);
            binding.refuseLin.setVisibility(View.VISIBLE);
            binding.OneVoiceLaunchLoading.setText(viewModel.bean.get().username + "想和你视频...");
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.refuse) {//拒绝和挂断
            ...
            if (binding.accept.getVisibility() == View.GONE) {
                ImMessageUtil.getInstance().sendCallMessage(viewModel.bean.get().userId, 1, 0, true);
            }
...
//        imApiOOOLive.init(socketClient);
    }
