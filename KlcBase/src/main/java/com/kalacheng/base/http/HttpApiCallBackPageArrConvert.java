...
public class HttpApiCallBackPageArrConvert<T> extends StringCallback {
    ...
    public HttpApiCallBackPageArrConvert(HttpApiCallBackPageArr<T> callback, Type retType) {
        m_retType = retType;
        m_callback = callback;
    }
...
//        ToastUtil.show("网络请求失败");
        m_callback.onHttpRet(44001, "", null, null);
    }
...
//                ARouter.getInstance().build(HttpApiCallBackConvert.LOGIN_AROUTER).navigation();
                ...
                m_callback.onHttpRet(retArr.getCode(), retArr.getMsg(), retArr.getPageInfo(), _retList);
            }
        } catch (Exception ex) {
            Log.i("HttpOnError", ex.getMessage());
            m_callback.onHttpRet(44002, ex.getMessage(), null, null);
            return;
        }
    }
