...
public class DateUtil {
     ...
     * 根据格式返回时间字符串
     *
     * @param sFormat 时间返回格式(如：yyyy-MM-dd HH:mm:ss SSSS 、 M月d日 、 H时m分)
     */
    ...
    // 返回月日
    public static String timeMinute1(String time) {
        SimpleDateFormat sdr = new SimpleDateFormat("M月d日");
    ...
    // 返回时分
    public static String timeMinute2(String time) {
        SimpleDateFormat sdr = new SimpleDateFormat("H时m分");
        @SuppressWarnings("unused")
        long lcc = Long.valueOf(time);
        int i = Integer.parseInt(time);
        String times = sdr.format(new Date(i * 1000L));
