...
public class VideoLocalUtil {
    ...
    public VideoLocalUtil() {
        mContentResolver = ApplicationUtil.getInstance().getContentResolver();
    ...
    private List<VideoChooseBean> getAllVideo() {
        ...
        try {
            cursor = mContentResolver.query(
                    ...
                    mediaColumns, null, null, MediaStore.Video.Media.DATE_MODIFIED + " desc");
            if (cursor != null) {
    ...
    /**
     * 把图片保存到ContentProvider,在选择上传的时候能找到
     */
    public static void savePicInfo(Context context, String picPath) {
        try {
            File videoFile = new File(picPath);
            String fileName = videoFile.getName();
            long currentTimeMillis = System.currentTimeMillis();
            ContentValues values = new ContentValues();
            values.put(MediaStore.MediaColumns.TITLE, fileName);
