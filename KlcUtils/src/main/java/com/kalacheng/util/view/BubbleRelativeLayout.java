...
public class BubbleRelativeLayout extends RelativeLayout {
    ...
    public static int SHADOW_COLOR = Color.argb(60, 0, 0, 0);
    ...
    private void init(final Context context, final AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.myBubble);

            try {
                PADDING = a.getDimensionPixelSize(R.styleable.myBubble_myPadding, PADDING);
                SHADOW_COLOR = a.getInt(R.styleable.myBubble_myShadowColor, SHADOW_COLOR);
                LEG_HALF_BASE = a.getDimensionPixelSize(R.styleable.myBubble_myHalfBaseOfLeg, LEG_HALF_BASE);
                MIN_LEG_DISTANCE = PADDING + LEG_HALF_BASE;
                STROKE_WIDTH = a.getFloat(R.styleable.myBubble_myStrokeWidth, STROKE_WIDTH);
                CORNER_RADIUS = a.getFloat(R.styleable.myBubble_myCornerRadius, CORNER_RADIUS);
            } finally {
                if (a != null) {
                    a.recycle();
                }
    ...
    private Matrix renderBubbleLegMatrix(final float width, final float height) {
        ...
        matrix.postTranslate(width-offset-15, dstY);
        return matrix;
    }
